...
//! crate's only feature is in `default` and it has no examples or benches,
//! but not the doctests. The repository used to carry an `act-validation.yml` workflow
...
//! - every declared feature is in `default`, and none is implied by an optional dependency, which
//!   is what makes `--all-features` the coverage default.
//!
...
/// Returns the entries of the manifest's `[features]` table as
/// `(name, value)` pairs, read line by line.
fn feature_entries(manifest: &str) -> Vec<(&str, &str)> {
    manifest
        .lines()
...
/// Returns `true` if every declared feature is in the `default` list, so
/// `--all-features` enables exactly what a default build does.
fn all_features_are_default(manifest: &str) -> bool {
    let entries = feature_entries(manifest);
    ...
    entries
        ...
        .all(|(name, _)| defaults.contains(&format!("\"{name}\"")))
}

#[test]
fn all_features_select_the_default_build() {
    ...
    assert!(
        all_features_are_default(&manifest) && !implicit,
        "a feature outside `default` makes --all-features differ from the coverage run; recheck \
         make test against coverage"
    );
}

#[rstest]
#[case::all_default("[features]\ndefault = [\"a\"]\na = []\n", true)]
#[case::extra("[features]\ndefault = [\"a\"]\na = []\nb = []\n", false)]
#[case::none("[package]\nname = \"x\"\n", true)]
fn feature_tables_are_read(#[case] manifest: &str, #[case] expected: bool) {
    assert_eq!(all_features_are_default(manifest), expected);
}
