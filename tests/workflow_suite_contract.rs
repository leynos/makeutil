...
/// Returns the manifest's `default` feature list.
fn default_features(manifest: &str) -> Result<Vec<String>, toml::de::Error> {
...
#[test]
fn every_feature_is_in_default() {
    let manifest = manifest_dir()
        .and_then(|dir| dir.read_to_string("Cargo.toml"))
        .expect("failed to read Cargo.toml");
    let defaults = default_features(&manifest).expect("Cargo.toml must parse as TOML");
    let outside: Vec<String> = manifest_features(&manifest)
        .expect("Cargo.toml must parse as TOML")
        .into_iter()
        .filter(|feature| feature != "default" && !defaults.contains(feature))
        .collect();
    assert!(
        outside.is_empty(),
        "{outside:?} are outside `default`, so --all-features differs from the coverage run"
    );
}
